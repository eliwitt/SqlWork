SELECT     reserve.id, reserve.CoverageID ResType, 
	...
	case when reserve.reserveopen =1 then 
		GoAutoProd.dbo.F_Claims_GetReserveAmount(reserve.CoverageID, cmast.policynumber, cmast.dateofloss, veh.ourvehicleindex)
	else 
	   0 end	Reserve,  
...
FROM clm_claimmaster  cmast
	left join clm_claimvehicle veh on cmast.id = veh.claimmasterptr and (veh.disabled = 0 or veh.disabled is null)
	join CLM_Reserve reserve on cmast.id = reserve.claimnumber and veh.entitynumber = reserve.entitynumber
...
FROM clm_claimmaster  cmast
    left join clm_vehiclepassenger pass on cmast.id = pass.claimmasterptr and (pass.disabled = 0 or pass.disabled is null)
	...
	join CLM_Reserve reserve on cmast.id = reserve.claimnumber and invperson.entitynumber = reserve.entitynumber 
       and coverageid in (2, 7, 8) 
    left join clm_claimvehicle veh on cmast.id = veh.claimmasterptr and veh.VehicleNumber = 1 and (veh.disabled = 0 or veh.disabled is null)
WHERE     cmast.id = 15
union 
SELECT     reserve.id, reserve.CoverageID ResType, (select [desc] from clm_coverage where id = reserve.CoverageID) ResTitle,
	case when reserveopen =1 then
	...
	case when reserve.amountpaid is null then 
	   ...
	   reserve.amountpaid end amountpaid,
   OpenedOn ResOpened, ClosedOn Resclosed, prop.entitynumber, substring(powner.firstname + ' ' + powner.lastname + ':' + prop.PropertyDesc, 0, 30) entitytitle, 'false' leinshow
FROM clm_claimmaster  cmast
	left join clm_otherproperty prop on cmast.id = prop.claimmasterptr and (prop.disabled = 0 or prop.disabled is null)
	...
	join CLM_Reserve reserve on cmast.id = reserve.claimnumber and prop.entitynumber = reserve.entitynumber
       and coverageid in (1, 3, 4, 5, 6, 9)
    left join clm_claimvehicle veh on cmast.id = veh.claimmasterptr and veh.VehicleNumber = 1 and (veh.disabled = 0 or veh.disabled is null)
WHERE     cmast.id = 15
union 
...
FROM clm_claimmaster  cmast
  	...
  	left join clm_involvedparty [owner] on Prop.involvedpartyptr = owner.id 
 	left join clm_person powner on owner.personptr = powner.id 
 	join CLM_Reserve reserve on cmast.id = reserve.claimnumber and [owner].entitynumber = reserve.entitynumber 
           and coverageid in (2, 7, 8) 
    left join clm_claimvehicle veh on cmast.id = veh.claimmasterptr and veh.VehicleNumber = 1 and (veh.disabled = 0 or veh.disabled is null)
where cmast.id = 15

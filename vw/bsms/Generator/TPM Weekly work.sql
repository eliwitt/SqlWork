...
--
--exec [wAppTPM].[TPMTestGenerator] '04/08/2013'
select COUNT(*) 'Total Open TPMs' from wo.WorkOrderMain where DISType = 3 AND DISCloseDate IS NULL
select COUNT(*) 'Total Incomplete TPMs' from wo.WorkOrderMain where DISType = 3 AND DISCloseDate = '2/7/1977'
...
select disid, 
	...
	DISTPMNumber
from wo.WorkOrderMain where DISType = 4 AND DISCloseDate IS NULL
--order by DISResponsible
--DISequip in (3895, 3896) and DISType = 3
--AND DISType = 3 AND DISCloseDate IS NULL
--ORDER BY DISOpenDate
--update wo.WorkOrderMain set DISCloseDate = '3/22/2013' where disid = 63724
...
--exec wAppTPM.TPMAutoGenerator
--SELECT * FROM WO.WorkOrderMain WHERE DISCloseDate Is Null AND DISTPMNumber in (803, 804)
--
--  frigure the daily WOs Actives
--
select pn.PROCID, pn.PROCMODEL, pn.PROCARG, ml.MLID, ml.MLPlantID,
PN.PROCMONTHGROUP, PN.PROCBIMONTHGROUP, PN.PROCQUARTGROUP, PN.PROCSEMIGROUP, PN.PROCANUALGROUP,
PN.PROCTWOYRGRUP, PN.PROCTHREEYRGROUP, PN.PROCFOURYRGROUP, PN.PROCEIGHTYRGROUP,

...
declare @wasteoftime nvarchar(max), @debug int = 3, @projDate datetime,
	@createDt datetime, @theprocessdate datetime, @theDuedate datetime, @dayofweek int
set @createDt = '07/22/2013'
set @theprocessdate = Convert(Date,DateAdd(Day, 7, @createDt))
...
if (@debug > 2)
begin
	select @createDt 'Open Date', @theprocessdate'Sch Date', @projDate 'Proj Date', @theDuedate 'Due Date', @dayofweek 'Day of week'
...
create table #schData (schKey int, schPlant int, schDescrip nvarchar(255), schAssignedTo int, schSinceLastScheduledInd smallint, schLastScheduledFreq int, 
...
insert into #schData
	select schkey,schPlant,schDescription,schAssignedTo,schSinceLastScheduledInd, schLastScheduledFreq, schLastScheduledDT,
	...
	((schSinceLastScheduledInd = 1 and ((schLastScheduledDT + isnull(schLastScheduledFreq, 0)) - isnull(schEarlyDays,0)) < @projDate) or
	(schSinceLastCompleteInd = 1 and ((schLastCompletedT + isnull(schLastCompletedFreq, 0)) - isnull(schEarlyDays, 0)) < @projDate)) and
...
-- 
update Scheduler.ChampsSchedules set schLastScheduledDT = (@theprocessdate + ISNULL(schEarlyDays, 0))
...
BEGIN TRY
    ...
    select sch.schkey, sch.schDescrip,
		...
		3, 4,
		@createDt, @theDuedate + ISNULL(sch.schEarly, 0),
...
BEGIN TRY
	...
	insert into sp.UsageList (ULListType, ULTableLink, ULSPLink, ULSPQtyUsed, ULSPQtyNeeded, 
		...
		where DISOpenDate = @createDt and DISType = 3
		order by disid	
END TRY
BEGIN CATCH
	-- using the new log table enter the error
...
BEGIN TRY
	...
	--
	update Scheduler.ChampsSchedules set schLastScheduledDT = @theprocessdate + ISNULL(schEarlyDays, 0)
	where schKey in (select distinct DISSchID from wo.WorkOrderMain where DISOpenDate = @createDt and DISType = 3)
END TRY
BEGIN CATCH
	-- using the new log table enter the error
	set @wasteoftime = cast(ERROR_NUMBER() as nvarchar(20)) + '==' + ERROR_MESSAGE()
...
begin
	...
	from wo.WorkOrderMain where DISOpenDate = @createDt and DISType = 3 order by DISSchID

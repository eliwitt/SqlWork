...
SELECT emp_person.Person_key employee_key, EMP_EmployeeProduct.costtablevalue_key,
	...
	EMP_Person.SSN_txt, EMP_Employee.Employee_ID, 
	EMP_Person.AddressLine1_txt, EMP_Person.AddressLine2_txt, EMP_Person.City_txt, 
	EMP_Person.PostalCode_txt, EMP_Person.HomePhone_txt, 
	EMP_EmployeeProduct.DeductionsPerYear_val, EMP_EmployeeProduct.PreTax_flg, 
	PRD_Product.Product_nm, MSC_Organization_1.Organization_nm, 
	PRD_Customization.ProductDisplay_nm, 
	EMP_EmployeeProduct.CoverageBenefit_txt, 
	EMP_EmployeeProduct.EmployeeYearlyProductCost_amt, 
	([EmployeeYearlyProductCost_amt]/[DeductionsPerYear_val]) AS Expr1, 
	EMP_EmployeeProduct.EmployerYearlyProductCost_amt, 
	[EmployerYearlyProductCost_amt]/[DeductionsPerYear_val] AS Expr2, 
	EMP_EmployeeEnrollmentPeriod.EnrollmentStatus_vv, 
	EMP_EmployeeProduct.EmployeeProductStatus_vv,
	'                       ' as TypeContract
into 		#EmpProd
FROM (MSC_Organization AS MSC_Organization_1 
	INNER JOIN (PRD_Product 
	INNER JOIN (	(EMP_EmployeeProduct INNER JOIN (
		(
		(
		(GRP_Group 
		...
		INNER JOIN EMP_EmployeeEnrollmentPeriod 
			ON (EMP_Employee.Person_key = EMP_EmployeeEnrollmentPeriod.Person_key) 
			AND (EMP_Person.Person_key = EMP_EmployeeEnrollmentPeriod.Person_key)
		) ON EMP_EmployeeProduct.Employee_key = EMP_Employee.Person_key) 
		INNER JOIN GRP_GroupProduct 
			ON (EMP_EmployeeProduct.GroupProduct_key = GRP_GroupProduct.GroupProduct_key) 
			AND (GRP_Group.Group_key = GRP_GroupProduct.Group_key)
		) ON PRD_Product.Product_key = GRP_GroupProduct.Product_key
		) ON MSC_Organization_1.Organization_key = PRD_Product.Carrier_key
		) INNER JOIN PRD_Customization 
			ON GRP_GroupProduct.GroupProduct_key = PRD_Customization.GroupProduct_key

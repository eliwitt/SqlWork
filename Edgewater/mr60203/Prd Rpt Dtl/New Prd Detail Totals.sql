...
set @Group_key = 'b2c6840d-0c98-4c17-880c-4f2122a69bbf,818b16ff-161e-4bf5-8d17-355d64510f7f'
--
...
create table #PrdDetail (
	...
	prods		int,
	totpremium	money,
	empcloseratio	decimal(5,3),
	avgpre		money,
	enrllprds	int,
	enrllpre	money,
	enrllcls	decimal(5,3),
	enrllavg	money,
	grptot		int,
	grpprds		int,
	grppre		money,
...
set @cmdStr = '
insert into #PrdDetail (group_key, user_key, product_key, parent, 
prdname, benefit, enrollmenttype, enrollfrom_dt, enrollthru_dt, rider, volprd, prods, totpremium, avgpre)
...
select e.group_key, u.user_key, prd.product_key, prd.product_key as parent, prd.product_nm, 
	...
	SUM( Case
		...
		Else (IsNull(EmployeeYearlyCost_amt, 0) + IsNull(EmployerYearlyCost_amt, 0)) - 
			 (IsNull(EmployeeStartCost_amt, 0)  + IsNull(EmployerStartCost_amt, 0)) 
	END )/count(distinct cast(e.person_key as varchar(36))) as avgpre
from emp_person p
...
--------------------------------------------------------------
--  This section performs totals for enrollers and groups
--------------------------------------------------------------
--
--  count the employees that the enroller has seen
--
set @cmdStr= '
update #PrdDetail set #PrdDetail.emptot=totemps.emptot
	from #PrdDetail
	join (SELECT gep.Group_key, u.user_key, count(distinct cast(e.person_key as varchar(36))) as emptot
		FROM EMP_Employee e
...
'
	...
	) totemps on totemps.group_key = #PrdDetail.group_key and
		#PrdDetail.user_key = totemps.user_key
'
if @debug > 2
	print 'Count the employees seen by each enroller ' + @cmdStr
else
	exec(@cmdStr)
---------------------------------------------------------------------------------------
-- we need to calculate the close ratio for each enroller get this total for each prod
-- then we will add the enroller's prod totals together to get the enroller total divied by
-- the number of different prods.  
-----------------------------------------------------------------------------------------
update #PrdDetail set empcloseratio = enrollers.closeratio 
from #PrdDetail
join(
	select group_key, user_key, emptot, prods, totpremium, prods/cast(emptot as decimal(3)) closeratio
	 from #PrdDetail 
) enrollers on #PrdDetail.group_key = enrollers.group_key and #PrdDetail.user_key = enrollers.user_key and
	#PrdDetail.prods = enrollers.prods

if @debug > 0
	select * from #PrdDetail
---------------------------------------------------------------------------
-- enrollers total for just the normal products
---------------------------------------------------------------------------
update #PrdDetail set enrllprds = enrollers.prods, enrllpre = enrollers.totpremium,
 enrllcls = enrollers.encls, enrllavg = enrollers.enavg
from #PrdDetail 	
join (
	select group_key, user_key,
		sum(prods) prods, 
		sum(totpremium) totpremium,
		sum(empcloseratio) / cast(count(distinct cast(product_key as varchar(36))) as decimal(3)) encls,
		sum(totpremium)/sum(prods) enavg
	from #PrdDetail
	where rider = 'N'
	group by group_key, user_key
) enrollers on #PrdDetail.group_key = enrollers.group_key and #PrdDetail.user_key = enrollers.user_key

if @debug > 0
	select * from #PrdDetail
		
--select distinct group_key, user_key, emptot, enrllprds, enrllpre, enrllcls from #PrdDetail
-------------------------------------------------------------------------
-- group totals once again just for normal products
--------------------------------------------------------------------------
update #PrdDetail set grpprds = grpenrs.entotprd, grppre = grpenrs.entotpre,
 grpcls = grpenrs.entotcls, grpavg = grpenrs.entotavg, grptot = grpenrs.entotseen
from #PrdDetail 	
join (
	select enrollers.group_key,
		sum(enrollers.emptot) entotseen,
		sum(enrollers.enrllprds) entotprd, 
		sum(enrollers.enrllpre) entotpre,
		sum(enrollers.enrllcls) / cast(count(distinct cast(enrollers.user_key as varchar(36))) as decimal(3)) entotcls,
		sum(enrollers.enrllpre)/sum(enrollers.enrllprds) entotavg
	from (
		select distinct group_key, user_key, emptot, enrllprds, enrllpre, enrllcls from #PrdDetail where rider = 'N'
	) enrollers
	group by enrollers.group_key
) grpenrs on #PrdDetail.group_key = grpenrs.group_key

---------------------------------------------------------------
--  Now collect everything and report it
--------------------------------------------------------------
if @debug < 3
begin
-- merge the product information with the group information
select #PrdDetail.group_key,  
	upper(grp.company_nm) Company_nm, upper(ido.Organization_nm) InternalDivision_nm, go.Logo_bin Logo,
	upper(go.Organization_nm) Group_nm, upper(go.OrganizationShort_nm) Group_ID, AccountType_vv,
	upper(Carrier.OrganizationShort_nm) Carrier, #PrdDetail.enrollmenttype as EnrollmentType_vv, 
	#PrdDetail.enrollfrom_dt, #PrdDetail.enrollthru_dt, 
	#PrdDetail.user_key,UPPER(mu.first_nm) First_nm, UPPER(mu.last_nm) Last_nm, 
	UPPER(Enroller_ID) Enroller_ID, #PrdDetail.product_key, parent, 
	upper(prdname) ProductDisplay_nm, Benefit, rider, volprd as VoluntaryProduct_flg, 
	prods as NumProducts, totemps as Employees_Seen, totpremium as TotalSubmittedPremium,
	enrllprds, enrllpre, enrllcls, enrllavg,
	grpprds, grppre, grpcls, grpavg, grptot
from #PrdDetail
	...
	-- Find the carrier of the product
	JOIN PRD_Product p ON #PrdDetail.Product_key = p.Product_key
	JOIN MSC_Organization Carrier ON Carrier.Organization_key = p.Carrier_key
ORDER BY #PrdDetail.group_key, Company_nm, #PrdDetail.user_key, parent, rider
end
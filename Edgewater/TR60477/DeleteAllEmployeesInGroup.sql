...
ALTER   PROCEDURE DeleteAllEmployeesInGroup
(
...
set @trapped = 0
if @debug > 0 
	print 'At the start of the Grp Delete process'
-------------------------------------------------------------------------------
...
IF (@@rowcount = 0)
begin
	set @ErrorMsg = @groupName + ' is an invalid Group Name resubmit with correct name.'
	goto Error_Handler
End
...
WHERE		GP.Group_key = @groupKey
UNION
SELECT 		E.Person_key
FROM		EMP_PERSON P
JOIN		EMP_EMPLOYEE E 
ON 		E.Person_key = P.Person_key
WHERE		E.Group_key = @groupKey


Select @EmpCount = Count(*) from @employeesToDelete

if (@EmpCount = 0)
begin
	set @ErrorMsg = 'No Employees to delete for ' + @groupName
	goto Error_Handler
end
if @debug > 0
begin
	print 'There are ' + cast(@EmpCount as nvarchar(5)) + 
		' employees to delete for ' + @groupName
	if @debug > 2
		select * from @employeesToDelete
end

PRINT 'Deleted '  + cast(@EmpCount as nvarchar(5)) + ' employees for ' + @groupName

--
-- start the grp transaction
--
begin tran GrpEmpTran
-- turn on the rollback indicator on 
set @trapped = 30
-------------------------------------------------------------------------------
-- delete Employee Product Data
...
IN 
 (SELECT EmployeeProduct_key
  ...
  JOIN	 @employeesToDelete E ON EP.Employee_key = E.Person_key)
if (@@error <> 0)
begin
	set @ErrorMsg = 'Error when deleting from ProductRider'
	goto Error_Handler
...
IN 
 (SELECT PriorEmployeeProduct_key
  FROM	 EMP_PriorEmployeeProduct PEP
  JOIN	 @employeesToDelete E ON PEP.Employee_key = E.Person_key)
if (@@error <> 0)
begin
...
if @debug = 4
begin
	rollback tran GrpEmpTran
	if @debug > 0 
		print 'Successfully deleted the employees trans rollbacked.'
end
else
begin
	commit tran GrpEmpTran
	if @debug > 0
		print 'Successfully deleted the employees trans committed.'
end
return
----------------------------------------------------------------------
-- Error Handler
